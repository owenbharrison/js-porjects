var lp1,lp2,p;

var radius = 25;

function setup(){
  createCanvas(400, 400);
  lp1 = createVector(random(height),random(width));
  lp2 = createVector(random(height),random(width));
  document.body.style.cursor = "none";
}

function draw(){
  background(170);
  line(lp1.x, lp1.y, lp2.x, lp2.y);
  var mv = createVector(mouseX, mouseY);
  var res = pointlineintersect(lp1, lp2, mv, radius);
  push();
  fill(!res*255,res*255,0);
  circle(mv.x, mv.y, radius);
  pop();
  push();
  textAlign(CENTER, CENTER);
  text(res, 200, 350);
  pop();
}

function pointlineintersect(a,b,p,s){
  var r=false;
  var θ=p5.Vector.sub(b,a).heading();
  for(var i=0;i<dist(a.x,a.y,b.x,b.y);i++){
    var v=p5.Vector.fromAngle(θ,i).add(a);
    if(dist(v.x,v.y,p.x,p.y)<s/2){r=true;}
  }
  return r;
}

function intersects(x,y,n,r){
  var t,e,c;
  return 0!==(t=(y.x-x.x)*(r.y-n.y)-(r.x-n.x)*(y.y-x.y))&&(c=((r.y-n.y)*(r.x-x.x)+(n.x-r.x)*(r.y-x.y))/t,e=((x.y-y.y)*(r.x-x.x)+(y.x-x.x)*(r.y-x.y))/t,0<c&&c<1&&0<e&&e<1)
}
