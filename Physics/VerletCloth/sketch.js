...
function setup(){
  ...
  for(var i=0;i<xlen;i++){
    var x = map(i, 0, xlen-1, width/4, width*3/4);
    for(var j=0;j<ylen;j++){
      var y = map(j, 0, ylen, 0, height/2);
      particles[i+j*xlen] = new Particle(x, y, j==0&&(i==0||i==xlen-1));
    }
  }
...
function draw(){
  ...
  gravity.x = controls.gravityX.value();
  gravity.y = controls.gravityY.value();
...
function setupControls(){
  if(!controls.beenSetup){
    ...
    controls.gravityX = createSlider(-1, 1, 0, 0.1).parent("#gravityX");
    controls.gravityY = createSlider(-1, 1, 0.5, 0.1).parent("#gravityY");
    ...
    controls.lockall = createButton("Freeze All Particles").parent("#lockall").mousePressed(()=>particles.forEach(p=>p.locked=true));
    controls.unlockall = createButton("Unfreeze All Particles").parent("#unlockall").mousePressed(()=>particles.forEach(p=>p.locked=false));
